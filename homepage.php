...
<head>
    ...
    <script>
        ...
        function hideAllStudentSections() {
            const sections = [
                ...
                'reservation', 'lab-resources', 'feedback', 'lab-schedules'
            ];
    ...
    <style>
        ...
        /* Style update for admin section titles to make them visible */
        .section-header h2, .admin-section h2 {
            color: #333 !important;
            background-color: white !important;
            ...
            <div class="admin-dashboard">
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows($active_sessions_result) > 0) {
                                    while ($session = mysqli_fetch_assoc($active_sessions_result)) {
                                        ...
                                        echo "<button onclick=\"endSitInSessionNoReward('" . $session['id'] . "')\" class='end-session-btn' style='background-color: #f8f8f8; color: black; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; margin-left: 5px;'>End Session</button>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
                                } else {
                ...
                <div id="computer-control" class="admin-section" style="display: none;">
                    ...
                    <div class="computer-control-container" style="background: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                        <p style="margin-bottom: 20px; text-align: center;">Manage PC availability status for laboratory rooms. This will affect which PCs students can reserve.</p>
...
<div class="lab-selection" style="margin-bottom: 20px; text-align: center;">
    ...
    <select id="controlLabSelector" onchange="loadControlComputers()" style="padding: 8px 15px; border-radius: 5px; border: 2px solid #E0B0FF; width: 300px; max-width: 100%;">
        ...
        <option value="MAC">MAC Lab</option>
                        ...
                        <div id="controlComputersGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 15px; margin-top: 20px;">
                        ...
                        <div class="lab-control-actions" style="margin-top: 20px; text-align: center;">
                            <p style="margin-bottom: 10px; color: #666; font-style: italic;">Click on a computer to lock or unlock it. Locked computers cannot be used for reservations or sit-ins.</p>
                </div>
                    </div>
                </div>
                </div>
        ...
        <?php } else { ?>
            ...
            <div id="main-content-container">
                ...
                <div id="lab-resources" class="section-placeholder" style="display: none; margin-top: 120px; overflow-y: visible; height: auto;">
                ...
                <div id="history" class="section-placeholder" style="display: none; margin-top: 120px; overflow-y: visible; height: auto;">
                    <h2 style="text-align: center; margin-bottom: 30px;">Your Sit-In History</h2>
                    <a href="#" class="back-btn" onclick="showStudentProfile(); return false;">Back to Profile</a>
                    
                    <div class="history-container" style="background-color: white; padding: 25px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); margin: 20px auto; max-width: 1000px;">
                        <div class="history-stats" style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 25px;">
                            <?php
                            // Get the student's ID from session
                            $student_id = $_SESSION['idno'];
                            
                            // Get statistics
                            $total_query = "SELECT COUNT(*) as total FROM sit_in_sessions WHERE student_id = '$student_id'";
                            $total_result = mysqli_query($conn, $total_query);
                            $total_sit_ins = mysqli_fetch_assoc($total_result)['total'];
                            
                            $active_query = "SELECT COUNT(*) as active FROM sit_in_sessions WHERE student_id = '$student_id' AND status = 'active'";
                            $active_result = mysqli_query($conn, $active_query);
                            $active_sit_ins = mysqli_fetch_assoc($active_result)['active'];
                            
                            $completed_query = "SELECT COUNT(*) as completed FROM sit_in_sessions WHERE student_id = '$student_id' AND status = 'completed'";
                            $completed_result = mysqli_query($conn, $completed_query);
                            $completed_sit_ins = mysqli_fetch_assoc($completed_result)['completed'];
                            
                            $duration_query = "SELECT SUM(duration) as total_minutes FROM sit_in_sessions WHERE student_id = '$student_id' AND status = 'completed'";
                            $duration_result = mysqli_query($conn, $duration_query);
                            $total_minutes = mysqli_fetch_assoc($duration_result)['total_minutes'];
                            ...
                            if ($minutes > 0) {
                                $time_text .= ($time_text ? ' ' : '') . $minutes . ' minute' . ($minutes > 1 ? 's' : '');
                            }
                            if (!$time_text) {
                                $time_text = '0 minutes';
                            }
                            ...
                            <div class="stat-card" style="flex: 1; min-width: 200px; background-color: white; color: white; padding: 20px; border-radius: 8px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                                <div class="stat-value" style="font-size: 28px; font-weight: bold;"><?php echo $total_sit_ins; ?></div>
                                <div class="stat-label">Total Sit-Ins</div>
                            </div>
                            
                            <div class="stat-card" style="flex: 1; min-width: 200px; background-color: white; color: white; padding: 20px; border-radius: 8px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                                <div class="stat-value" style="font-size: 28px; font-weight: bold;"><?php echo $active_sit_ins; ?></div>
                                <div class="stat-label">Active Sit-Ins</div>
                            </div>
                            
                            <div class="stat-card" style="flex: 1; min-width: 200px; background-color: white; color: white; padding: 20px; border-radius: 8px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                                <div class="stat-value" style="font-size: 28px; font-weight: bold;"><?php echo $completed_sit_ins; ?></div>
                                <div class="stat-label">Completed Sit-Ins</div>
                            </div>
                            
                            <div class="stat-card" style="flex: 1; min-width: 200px; background-color: white; color: white; padding: 20px; border-radius: 8px; text-align: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                                <div class="stat-value" style="font-size: 28px; font-weight: bold;"><?php echo $time_text; ?></div>
                            ...
                            <table class="history-table" style="width: 100%; border-collapse: collapse;">
                                <thead>
                                    <tr style="background-color: #f5f5f5;">
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">ID</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">Laboratory</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">Purpose</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">Start Time</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">End Time</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">Duration</th>
                                        <th style="padding: 12px; text-align: left; border-bottom: 2px solid #E0B0FF;">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    // Get the student's sit-in history
                                    $history_query = "SELECT * FROM sit_in_sessions WHERE student_id = '$student_id' ORDER BY session_start DESC";
                                    $history_result = mysqli_query($conn, $history_query);
                                    
                                    if (mysqli_num_rows($history_result) > 0) {
                                        while ($row = mysqli_fetch_assoc($history_result)) {
                                            $start_time = date('M d, Y h:i A', strtotime($row['session_start']));
                                            $end_time = $row['session_end'] ? date('M d, Y h:i A', strtotime($row['session_end'])) : '-';
                                            ...
                                            if ($row['status'] == 'active') {
                                                $status_class = 'background-color: #8fda8f; color: white; padding: 5px 10px; border-radius: 20px; font-size: 12px;';
                                            } else if ($row['status'] == 'completed') {
                                                $status_class = 'background-color: #ff8080; color: white; padding: 5px 10px; border-radius: 20px; font-size: 12px;';
                                            } else {
                                                $status_class = 'background-color: #80c1ff; color: white; padding: 5px 10px; border-radius: 20px; font-size: 12px;';
                                            }
                                            
                                            echo '<tr style="border-bottom: 1px solid #eee; '.$row_color.'">';
                                            echo '<td style="padding: 12px;">'.$row['id'].'</td>';
                                            echo '<td style="padding: 12px;">Lab '.$row['laboratory'].'</td>';
                                            echo '<td style="padding: 12px;">'.$row['purpose'].'</td>';
                                            echo '<td style="padding: 12px;">'.$start_time.'</td>';
                                            echo '<td style="padding: 12px;">'.$end_time.'</td>';
                                            echo '<td style="padding: 12px;">'.$duration.'</td>';
                                            echo '<td style="padding: 12px;"><span style="'.$status_class.'">'.$row['status'].'</span></td>';
                                            echo '</tr>';
                                        }
                                    } else {
                                        echo '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #666;">No sit-in history found.</td></tr>';
                                    }
                                    ?>
                                </tbody>
                            </table>
    ...
    <!-- Edit Profile Modal -->
    <div id="editProfileModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4); padding-top: 60px;">
        <div class="modal-content" style="background-color: #fefefe; margin: 5% auto; padding: 20px; border: 1px solid #888; width: 80%; max-width: 600px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            <span class="close" onclick="closeEditProfileModal()" style="color: #aaa; float: right; font-size: 28px; font-weight: bold; cursor: pointer;">&times;</span>
            <h2 style="margin-bottom: 20px; color: #E0B0FF; text-align: center;">Edit Profile</h2>
            
            <form action="update_profile.php" method="post" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 15px;">
                <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 250px;">
                        <label for="firstname" style="display: block; margin-bottom: 5px;">First Name:</label>
                        <input type="text" id="firstname" name="firstname" value="<?php echo $_SESSION['user']; ?>" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                    
                    <div style="flex: 1; min-width: 250px;">
                        <label for="midname" style="display: block; margin-bottom: 5px;">Middle Name:</label>
                        <input type="text" id="midname" name="midname" value="<?php echo isset($user_data['midname']) ? $user_data['midname'] : ''; ?>" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                </div>
                ...
                <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 250px;">
                        ...
                        <input type="text" id="lastname" name="lastname" value="<?php echo isset($user_data['lastname']) ? $user_data['lastname'] : ''; ?>" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                    
                    <div style="flex: 1; min-width: 250px;">
                        <label for="email" style="display: block; margin-bottom: 5px;">Email:</label>
                        <input type="email" id="email" name="email" value="<?php echo isset($user_data['email']) ? $user_data['email'] : ''; ?>" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                </div>
                ...
                <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 250px;">
                        ...
                        <select id="course" name="course" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                            <option value="1" <?php echo (isset($user_data['course']) && $user_data['course'] == 1) ? 'selected' : ''; ?>>BS Information Technology</option>
                            <option value="2" <?php echo (isset($user_data['course']) && $user_data['course'] == 2) ? 'selected' : ''; ?>>BS Computer Science</option>
                            <option value="3" <?php echo (isset($user_data['course']) && $user_data['course'] == 3) ? 'selected' : ''; ?>>BS Information Systems</option>
                        </select>
                    </div>
                    
                    <div style="flex: 1; min-width: 250px;">
                        <label for="yearlvl" style="display: block; margin-bottom: 5px;">Year Level:</label>
                        <select id="yearlvl" name="yearlvl" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                            <option value="1" <?php echo (isset($user_data['yearlvl']) && $user_data['yearlvl'] == 1) ? 'selected' : ''; ?>>1st Year</option>
                            <option value="2" <?php echo (isset($user_data['yearlvl']) && $user_data['yearlvl'] == 2) ? 'selected' : ''; ?>>2nd Year</option>
                            <option value="3" <?php echo (isset($user_data['yearlvl']) && $user_data['yearlvl'] == 3) ? 'selected' : ''; ?>>3rd Year</option>
                            <option value="4" <?php echo (isset($user_data['yearlvl']) && $user_data['yearlvl'] == 4) ? 'selected' : ''; ?>>4th Year</option>
                        </select>
                    </div>
                </div>
                ...
                <div style="margin-top: 10px;">
                    <label for="photo" style="display: block; margin-bottom: 5px;">Profile Photo:</label>
                    <input type="hidden" name="current_photo" value="<?php echo isset($user_data['photo']) ? $user_data['photo'] : ''; ?>">
                    
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <div style="flex: 0 0 100px;">
                            <?php if (isset($user_data['photo']) && !empty($user_data['photo'])): ?>
                                <img src="uploads/<?php echo $user_data['photo']; ?>" alt="Profile Photo" style="width: 100px; height: 100px; object-fit: cover; border-radius: 50%;">
                            <?php else: ?>
                                <div style="width: 100px; height: 100px; background-color: #eee; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                        ...
                        <div style="flex: 1;">
                            <div class="custom-file-upload" style="position: relative; overflow: hidden; display: inline-block; cursor: pointer;">
                                <input type="file" name="photo" id="photo" accept="image/*" style="font-size: 100px; position: absolute; left: 0; top: 0; opacity: 0; cursor: pointer;" onchange="updateFileName(this)">
                                <button type="button" style="padding: 8px 15px; background-color: #E0B0FF; color: white; border: none; border-radius: 4px; cursor: pointer;">Choose File</button>
                                <span id="file-name-display" style="margin-left: 10px; font-size: 14px;">No file chosen</span>
                            </div>
                            <small style="display: block; margin-top: 5px; color: #666;">Max file size: 500KB. Allowed formats: JPG, JPEG, PNG, GIF</small>
                        </div>
                    </div>
                </div>
                
                <div style="margin-top: 20px; text-align: center;">
                    <button type="submit" style="padding: 10px 20px; background-color: #E0B0FF; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Save Changes</button>
                    <button type="button" onclick="closeEditProfileModal()" style="padding: 10px 20px; background-color: #ddd; color: #333; border: none; border-radius: 4px; cursor: pointer; margin-left: 10px;">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('Lab resources section exists:', !!document.getElementById('lab-resources'));
            ...
            window.showStudentLabResources = function() {
                ...
                hideAllStudentSections();
                document.getElementById('lab-resources').style.display = 'block';
                return false;
            };
