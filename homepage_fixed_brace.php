    ...
    <div class="content" id="content">
        <?php if (isset($_SESSION['is_admin']) && $_SESSION['is_admin']) { } ?>
            ...
            <div class="admin-dashboard">
                ...
                <h3 id="computer-control-heading" style="margin-bottom: 20px; color: #E0B0FF;">Computer Control</h3>
...
<div class="computer-status-legend" style="display: flex; justify-content: center; gap: 20px; margin-bottom: 20px;">
    <div class="status-item" style="display: flex; align-items: center; gap: 5px;">
        <div class="color-box" style="width: 20px; height: 20px; border-radius: 4px; background-color: #8fda8f;"></div>
        <span>Vacant</span>
        </div>
    <div class="status-item" style="display: flex; align-items: center; gap: 5px;">
        <div class="color-box" style="width: 20px; height: 20px; border-radius: 4px; background-color: #ff8080;"></div>
        <span>Occupied</span>
    </div>
    <div class="status-item" style="display: flex; align-items: center; gap: 5px;">
        <div class="color-box" style="width: 20px; height: 20px; border-radius: 4px; background-color: #e0b0ff;"></div>
        <span>Reserved</span>
                </div>
    <div class="status-item" style="display: flex; align-items: center; gap: 5px;">
        <div class="color-box" style="width: 20px; height: 20px; border-radius: 4px; background-color: #888888;"></div>
        <span>Locked</span>
                </div>
                </div>
...
function loadControlComputers() {
    ...
    xhr.onload = function() {
        if (this.status === 200) {
            try {
                console.log("Raw response:", this.responseText); // Debug
                
                // Parse the response JSON
                const computers = JSON.parse(this.responseText);
                console.log("Parsed computers:", computers); // Debug
                ...
                document.getElementById('controlComputersGrid').innerHTML = 
                    ...
                    '<p>Response text: ' + this.responseText.substring(0, 100) + '...</p></div>';
            }
                } else {
            document.getElementById('controlComputersGrid').innerHTML = 
                '<div class="placeholder-message" style="grid-column: 1/-1; text-align: center; padding: 50px;">' +
...
function toggleComputerStatus(labId, computerId) {
    if (!confirm('Are you sure you want to toggle the status of PC ' + computerId + ' in Lab ' + labId + '?')) {
        return;
    }
...
<!-- End Computer Control Section -->argin-right: 20px;">Lab Status</h3>
                    <div style="display: flex; align-items: center;">
